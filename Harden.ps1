...
function Document-System {
    ...
    <#
.SYNOPSIS
    Search all user Documents folders for files that look like plaintext username/password lists.

.DESCRIPTION
    - Scans user profile folders under C:\Users (skips common system profiles).
    - Searches common text extensions and .docx (by reading document.xml inside the .docx ZIP).
    - Uses regex to detect obvious secrets: lines containing "password"/"pwd"/"pass" or username:password-style lines.
    - Prints absolute file paths for matches and writes a results CSV (optional).

.NOTES
    Run as Administrator.
#>

function Find-CredentialFilesInDocuments {
    param(
        [string]$UserProfilesRoot = 'C:\Users',
        [string]$OutputCsv = "$env:USERPROFILE\Documents\credential-find-results.csv",
        [switch]$SaveCsv
    )

    # Profiles to skip
    $skipNames = @('All Users','Default','Default User','DefaultAppPool','Public','desktop.ini','WDAGUtilityAccount')

    # File extensions to scan as text
    $textExts = @('.txt', '.log', '.csv', '.ini', '.cfg', '.md', '.json', '.xml', '.html', '.htm', '.ps1')

    # Regex patterns to detect credentials (case-insensitive)
    $patterns = @(
        '(?i)\b(password|pwd|pass|passwd)\b',                                           # contains password-like word
        '(?m)^\s*[\w\.\-@\\\/]+(?:\s|:|=|,|\t)+\S{6,}\s*$',                           # username : password (password min length 6)
        '(?m)^\s*[\w\.\-@]+[:=]\s*[^ \t]{6,}\b'                                       # user:pass on a single line
    )

    $matches = @()

    # Helper: test text content against patterns
    function Test-TextForCredentials {
        param($Text,$FilePath)
    ...
    # Helper: extract text from .docx (reads word/document.xml)
    function Get-DocxText {
        param($Path)
        try {
            # .docx is a zip archive; use System.IO.Compression.ZipFile if available
            Add-Type -ErrorAction SilentlyContinue -AssemblyName System.IO.Compression.FileSystem
            $tempDir = Join-Path $env:TEMP ("docx_extract_{0}" -f ([guid]::NewGuid().ToString()))
    ...
    $profileDirs = Get-ChildItem -Path $UserProfilesRoot -Directory -ErrorAction SilentlyContinue |
    ...
    foreach ($profile in $profileDirs) {
        ...
        if (-not (Test-Path $documentsPath)) { continue }

        # Get candidate files
        $files = Get-ChildItem -Path $documentsPath -Recurse -File -ErrorAction SilentlyContinue |
                 Where-Object {
                     # include text-like extensions or .docx
                     $textExts -contains $_.Extension.ToLower() -or $_.Extension.ToLower() -eq '.docx'
                 }
        ...
        foreach ($f in $files) {
            ...
            try {
                if ($ext -eq '.docx') {
                    $txt = Get-DocxText -Path $f.FullName
                    if ($txt -and (Test-TextForCredentials -Text $txt -FilePath $f.FullName)) {
                        $found = $true
                    }
                } else {
                    # try reading as UTF8; fallback to default encoding if necessary
                    $content = $null
                    try {
                    ...
                    if ($content -and (Test-TextForCredentials -Text $content -FilePath $f.FullName)) {
                        $found = $true
                    }
                }
            } catch {
            ...
            if ($found) {
                $result = [PSCustomObject]@{
                    ...
                    MatchedFile    = $f.FullName
                    SizeBytes      = $f.Length
                    LastWriteTime  = $f.LastWriteTime
                }
                $matches += $result
                # Print absolute path immediately
                Write-Host "MATCH: $($f.FullName)" -ForegroundColor Yellow
            }
        }
    }

    if ($SaveCsv -and $matches.Count -gt 0) {
        $matches | Export-Csv -Path $OutputCsv -NoTypeInformation -Force
        Write-Host "Results saved to: $OutputCsv"
    }

    if ($matches.Count -eq 0) {
        Write-Host "No likely credential files found in user Documents folders." -ForegroundColor Green
    }

    return $matches
}

# Run the function (save results to CSV as an example)
$found = Find-CredentialFilesInDocuments -SaveCsv

# Print a short summary
if ($found.Count -gt 0) {
    Write-Host "`nSummary of matched files:" -ForegroundColor Cyan
    $found | Select-Object Profile, MatchedFile, SizeBytes, LastWriteTime | Format-Table -AutoSize
} else {
    Write-Host "`nNo matches discovered." -ForegroundColor Green
}
